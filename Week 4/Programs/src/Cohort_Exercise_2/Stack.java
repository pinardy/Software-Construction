...
public class Stack<E> extends Vector<E> {
	...
	public boolean repOK() {
		...
		if (this.empty() != this.elements().hasMoreElements()) {
			System.out.println("this empty != this elements has more elements");
			return false;
		}

		// empty and size is not 0
		if (this.isEmpty() && this.size() != 0) {
			System.out.println("this.isEmpty() && this.size() != 0");
			return false;
		}

		// capacity is smaller than size
		if (this.capacity() < this.size()) {
			System.out.println("this.capacity() < this.size()");
			return false;
		}
