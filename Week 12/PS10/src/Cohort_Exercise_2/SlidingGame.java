...
//    	int[] initialBoardConfig = new int[] {2,3,8,4,6,0,1,5,7};
		...
		Latch<PuzzleNode> solution = new Latch<>();
...
//		List<int[]> trace = BFSSearch(new PuzzleNode(initialBoardConfig, null));
        BFSSearch(exec, solution, taskCount, seen, new PuzzleNode(initialBoardConfig, null));
...
//    }
    ...
    public static void BFSSearch(final ExecutorService exec, final Latch<PuzzleNode> solution, final AtomicInteger taskCount, final ConcurrentMap<String, Boolean> seen, final PuzzleNode node) {
        taskCount.incrementAndGet();
        exec.execute(new Runnable () {

            public void run() {
                if (solution.isSet() || seen.putIfAbsent(SlidingGame.toString(node.config), true) != null) {
                ...
                if (isGoal(node.config)) {
                    ...
                    for (int[] next: nextPositions(node.config)) {
                        BFSSearch(exec, solution, taskCount, seen, new PuzzleNode(next, node));
                    }
                }
            }
        });
